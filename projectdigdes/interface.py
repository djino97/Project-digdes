...
class Example(QWidget):
    ...
    def initUI(self):
        ...
        btn_input.clicked.connect(self.get_supply)

        lbl_supply = QLabel("Точка поставки", self)
        lbl_supply.move(100, 25)
        lbl_consupmtion = QLabel("Точка поставки", self)
    ...
    def get_supply(self):
        ...
        id_supply, num_party = session_create2(weight=weight, supply=supply, date=date_departure, loading=loading)
        unloading = unloading_party(weight)
        id_consumption = session_add_consumption(consumption=consumption, date=date_arrival, unloading=unloading,
                                                 party=num_party)
        update_graphic(id_supply)
